...
<div class="panel panel-default">
    <div class="panel-heading">
        <?php echo language("financials-heading", $_SESSION['lang']); ?>
    </div>
        ...
        <table class="table table-striped table-responsive table-hover text-center">
            <thead>
                <tr>
                    ...
                    <th><?php echo language('financials-description', $_SESSION['lang']); ?></th>
                    <th><?php echo language('financials-amount', $_SESSION['lang']); ?></th>
                    <th><?php echo language('financials-article', $_SESSION['lang']); ?></th>
                    <th><?php echo language('financials-date', $_SESSION['lang']); ?></th>
                    <th><?php echo language('financials-manage', $_SESSION['lang']); ?></th>
                </tr>
            </thead>
            <tbody>
                <?php
                $query = "SELECT * FROM financialAids";
                ...
                while($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                    ...
                    <tr>
                        ...
                        <td>
                        <a href="financials.php?lang=<?php echo $selectedLang; ?>&manage=edit&id=<?php echo $row['id']; ?>">
    ...
    <div class="panel-footer">
        <a href="financials.php?lang=<?php echo $selectedLang; ?>&manage=add" class="btn btn-primary btn-lg">
...
?>
    <!-- ADDING FINANCIAL AID -->
    <div class="panel panel-primary">
        <div class="panel-heading">
            <?php echo language('financials-add', $_SESSION['lang']); ?>
        </div>
        <div class="panel-body">
            <form action="financials.php?manage=store" method="POST" data-parsley-validate="">
                <div class="form-group">
                    <label for="description"><?php echo language('financials-description', $_SESSION['lang']); ?></label>
                    <input type="text" placeholder="<?php echo language('financials-description', $_SESSION['lang']); ?>"
                        value="<?php echo isset($_SESSION['description']) ? $_SESSION['desciption'] : ''; ?>"  data-parsley-maxlength="255"
                        class="form-control" name="description"
                    />
                </div>
                <div class="form-group">
                    <label for="amount"><?php echo language('financials-amount', $_SESSION['lang']); ?></label>
                    <input type="text" placeholder="<?php echo language('financials-amount', $_SESSION['lang']); ?>"
                ...
                <div>
                    ...
                    ?>
                        <label for="article_id"><?php echo language('financials-article', $_SESSION['lang']); ?></label>
                        <select name="article_id" class="form-control">
                            <?php
                ...
                <div class="bootstrap-iso">
                    <div class="form-group">
                        <label for="date"><?php echo language("financials-date", $_SESSION['lang']); ?></label>
...
***************************************************************/
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        if (isset($_POST['amount']) && isset($_POST['date'])) {
            ...
            //     $_SESSION['error'] = language('financials-desc-length-error', $_SESSION['lang']);
            //     header('Location: financials.php?manage=add&lang='.$selectedLang);
            //     die();
            // }
            // amount Validation
            ...
            try {
                $query = "INSERT INTO financialAids (description, amount, article_id, date)
                ...
                $_SESSION['error'] = language('financials-add-error', $_SESSION['lang']);
                dbError($stmt, 'financials.php?manage=add&lang='.$selectedLang);
                die();
            }
            $_SESSION['success'] = language('financials-add-success', $_SESSION['lang']);
            header('Location: financials.php?manage=view&lang='.$selectedLang);
            die();
        }
        else {
            $_SESSION['error'] = language('financials-required', $_SESSION['lang']);
            header('Location: financials.php?manage=add&lang='.$selectedLang);
            die();
        }
    }
...
?>
    ...
    if (isset($_GET['id'])) {
        ...
        $query = "SELECT * FROM financialAids WHERE id = :id";
        $stmt = Connection::conn()->prepare($query);
        ...
        <div class="panel panel-primary">
            ...
            <div class="panel-body">
                <form action="financials.php?manage=update" method="POST" data-parsley-validate="">
        ...
        <?php } else {
            ...
            header('financials.php?lang='.$selectedLang);
            die();
        }
    }
    else {
        $_SESSION['error'] = language('id-required', $_SESSION['lang']);
        header('Location: financials.php?lang='.$selectedLang);
        die();
    }
...
***************************************************************/
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        if (isset($_POST['amount']) && isset($_POST['date'])) {
            ...
            // if (strlen($description) > 255) {
            //     $_SESSION['error'] = language('financials-desc-length-error', $_SESSION['lang']);
            //     header('Location: financials.php?manage=edit&id='.$_POST['id'].'&lang='.$selectedLang);
            //     die();
            // }
            // amount Validation
            $amount = filter_var(testInput($_POST['amount']), FILTER_VALIDATE_FLOAT);
            ...
            $id = $_POST['id'];
            try {
                $query = "UPDATE financialAids SET description = :description, amount = :amount,
                ...
                $_SESSION['error'] = language('financials-add-error', $_SESSION['lang']);
                dbError($stmt, 'financials.php?manage=edit&id='.$_POST['id'].'&lang='.$selectedLang);
                die();
            }
            $_SESSION['success'] = language('financials-add-success', $_SESSION['lang']);
            header('Location: financials.php?manage=view&lang='.$selectedLang);
...
***************************************************************/
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        if (isset($_POST['id'])) {
            ...
            try {
                $query = "DELETE FROM financialAids WHERE id = :id";
                $stmt = Connection::conn()->prepare($query);
                $stmt->bindParam(':id', $id, PDO::PARAM_INT);
                $stmt->execute();
                $_SESSION['success'] = language("financials-delete-success", $_SESSION['lang']);
                header('Location: financials.php?manage=view&lang='.$selectedLang);
                die();
            } catch (PDOException $e) {
                dbError($stmt, 'financials.php?manage=view&lang='.$selectedLang);
                die();
            }
        }
        else {
